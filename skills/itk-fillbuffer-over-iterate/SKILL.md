---
name: itk-fillbuffer-over-iterate
description: >-
  Use when an ITK image is blanked to a constant by an iterator fill loop —
  a for/while loop over an ImageRegionIterator (or ImageRegionIteratorWithIndex)
  whose only body is `it.Set(C)` or `*it = C` for a loop-invariant constant C
  over the image's full buffered/largest-possible region, with the index
  otherwise unused. Replace the whole loop with `image->FillBuffer(C)`: a
  contiguous std::fill that is faster and drops the iterator-header dependency.
  Keywords: FillBuffer, iterator fill loop, allocate-and-blank, ImageRegionIterator
  Set constant, fill image with value, ITK PERF, performance, contiguous fill,
  remove iterator typedef, GetBufferedRegion fill.
---
...
- The image is fully allocated before the original loop (FillBuffer requires an
  allocated buffer — same precondition the loop had).
- Rebuild the consumer (ccache keeps it fast); confirm the object/library
  artifact exists, not just a zero pipe exit.
- If tests exist for the touched module, run them — pixel values must be
  identical to the loop version.
