---
name: itk-ctad-iterator
description: >-
  Use when modernizing ITK C++ source that constructs image iterators with
  redundant explicit template arguments that C++17 CTAD (class template argument
  deduction) can deduce from the image argument. Targets temporary/constructor
  sites like `ImageRegionIterator<OutputImageType>(this->GetOutput(), face)`
  where the `<...>` is deletable because the first constructor arg is an image.
  Closed set of iterator class names: ImageRegionIterator,
  ImageRegionConstIterator, ImageRegionIteratorWithIndex,
  ImageRegionConstIteratorWithIndex, ImageScanlineIterator,
  ImageScanlineConstIterator, ImageRegionRange, ImageBufferRange. Keywords: CTAD,
  class template argument deduction, deduction guide, redundant template args,
  image iterator construction, drop explicit template arguments, C++17 modernize,
  ITK iterator idiom.
---
...
- Re-run `detect.sh` — count should drop by the number of accepted edits.
- `git diff` — every change is a pure deletion of a `<...>` token, nothing else.
- Build the tree (CTAD failures are compile errors, so the build is the proof
  that deduction succeeded): `pixi run build-ITK`.
- `pre-commit run clang-format --all-files` clean.
