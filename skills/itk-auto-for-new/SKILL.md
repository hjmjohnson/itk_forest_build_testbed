---
name: itk-auto-for-new
description: >-
  Use when modernizing ITK or ITK-consumer C++ to replace the redundant
  smart-pointer declaration boilerplate `FooType::Pointer x = FooType::New();`
  (and the explicit-Pointer-alias form `const FooPointer x = FooType::New();`)
  with `auto x = FooType::New();`. The left-hand type repeats the right-hand
  `::New()` factory type verbatim, so `auto` is exact and lossless. Triggers:
  reviewer asks for "use auto" / modernize-use-auto on `::New()` sites, Pointer
  type repeated on both sides of `=`, redundant SmartPointer declarations,
  itk::SmartPointer<...> spelled out, clang-tidy modernize-use-auto for ITK.
  Keywords: ::Pointer = ::New(), auto reader, ITK smart pointer auto, New()
  declaration, modernize-use-auto, FooPointer alias.
---
...
```bash
bash skills/itk-auto-for-new/detect.sh <repo>     # count should drop after --apply
bash skills/itk-compile-gate.sh <build-dir> --changed <repo>   # warning-fatal gate
cmake --build <build-dir>                          # full build must still compile
pre-commit run --all-files                          # clang-format clean
```

The compile is the real proof: if `auto` deduced a different type the build
would fail or behavior would change. A clean build over a rewritten tree is the
artifact, not the regex match count.

Run `itk-compile-gate.sh` before committing: a plain build with default flags
will pass an orphaned alias, but the gate makes `-Wunused-local-typedefs`
(and set-but-unused locals) fatal and prefers GCC, matching the dashboards that
treat warnings as errors. The orphan sweep removes the common case; the gate
catches any the sweep leaves (e.g. an alias defined twice in one file).
