---
name: itk-equals-default-special-members
description: >-
  Use when an ITK (or ITK-consumer) class has hand-written empty-body special
  members that should be `= default` — a default constructor `Foo() {}`, a
  destructor `~Foo() override {}`, or an out-of-line `Foo::~Foo() {}` whose body
  is empty. These dead empty bodies suppress triviality and move semantics and
  litter consumer `.hxx`/`.h` files. Symptoms/keywords: empty-body constructor,
  empty-body destructor, `{}` special member, modernize-use-equals-default,
  out-of-line empty destructor, `T::~T() {}`, restore move semantics, restore
  triviality, dead default ctor/dtor, ITK equals-default campaign. NOT for
  polymorphic destructors that must stay out-of-line for ABI/visibility (those
  belong to itk-inline-destructors-fix).
---
...
# itk-equals-default-special-members
...
```bash
git diff                          # every hunk is {} -> = default, no logic lost
bash skills/itk-equals-default-special-members/detect.sh   # in-class count -> 0
cmake --build <build-dir>         # compiles; no new -Wdeprecated/ABI breaks
```

Confirm by artifact (the rebuilt library exists), not the build pipe's exit
code. For exported classes, double-check no hidden-symbol/ABI regression was
introduced (run `itk-inline-destructors-fix`'s audit if in doubt).
